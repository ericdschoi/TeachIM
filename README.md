...
- **▶ Presenter guide** (was "Teach mode") — the PRESENTER's private
  full-screen step-through, designed to be opened on your phone while the
  students watch the slides on a separate screen. One segment, case, or MCQ per
...
- **Student screen** — the lesson page groups the slide options under
  "Student screen": put these on a display the students can see, separate
  from your phone. Try **▶ PowerPoint online** first for maximal effect
  (hosted https site only: opens the original pptx in Microsoft's free Office
  viewer, decks ≤10 MB, closest to true PowerPoint rendering). Fallbacks:
  **▶ Slides in-app (PDF)** — full-screen in-browser viewer (swipe/arrow,
  wake-lock) rendering each lesson's `*_slides.pdf` (auto-generated from the
  pptx; regenerate missing ones with LibreOffice:
