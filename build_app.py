...
[data-theme="dark"] .pill{background:var(--pt);color:#fff}
...
@media(hover:hover) and (pointer:fine){.chips{flex-wrap:wrap;overflow-x:visible}}
...
.chip .ct{opacity:.65;margin-left:5px;font-size:.78em}
.chip.tagc{background:var(--pb);color:var(--pt);border-color:transparent}
.chip.tagc.on{box-shadow:0 0 0 2px var(--acc);font-weight:700}
[data-theme="dark"] .chip.tagc{background:var(--pt);color:#fff}
.hero{display:flex;align-items:flex-end;justify-content:space-between;gap:14px;flex-wrap:wrap}
.hero .stats{color:var(--mut);font-size:.92rem}
.prog{flex:0 1 250px;min-width:200px}
.prog .lbl{display:flex;justify-content:space-between;font-size:.76rem;color:var(--mut);margin-bottom:4px}
.prog .bar{height:6px;border-radius:999px;background:var(--line);overflow:hidden;display:flex}
.prog .bar i{display:block;height:100%}
.shead{font-size:.78rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--mut);
  margin:22px 2px 2px;display:flex;align-items:baseline;gap:8px}
.shead .cnt{font-weight:400}
.upnext{background:var(--card);border:1px solid var(--line);border-left:4px solid var(--warm);border-radius:0 12px 12px 0;
  padding:11px 14px;margin-top:8px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;cursor:pointer;box-shadow:var(--shadow)}
.upnext .t{font-weight:600;font-size:.95rem}
@media(hover:hover){.card{transition:transform .12s,box-shadow .12s,border-color .12s}
  .card:hover{transform:translateY(-2px);border-color:var(--acc);box-shadow:0 4px 8px rgba(15,30,40,.1),0 12px 32px rgba(15,30,40,.12)}}
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:14px;margin-top:12px}
.card{background:var(--card);border:1px solid var(--line);border-left:4px solid var(--tc,var(--line));
...
.actions{display:flex;gap:8px;flex-wrap:wrap;margin:6px 0 4px}
.agroup{margin-top:14px}
.aglabel{font-size:.74rem;font-weight:700;color:var(--mut);text-transform:uppercase;letter-spacing:.06em}
.setuphint{margin:14px 0 2px;padding:11px 14px;border-radius:11px;background:var(--accSoft);
  border:1px solid var(--line);font-size:.86rem;color:var(--mut);line-height:1.5}
.setuphint b{color:var(--ink)}
...
function pill(tag){const c=TAGCOL[tag]||TAGCOL.General;
 return '<span class="pill" style="--pb:'+c[0]+';--pt:'+c[1]+'">'+esc(tag)+'</span>';}
...
function renderHome(){
  ...
  const nT=DATA.filter(l=>(prog[l.folder]||{}).status==="taught").length;
  const nP=DATA.filter(l=>(prog[l.folder]||{}).status==="planned").length;
  const nM=DATA.reduce((a,l)=>a+l.mcqs.length,0),nC=DATA.reduce((a,l)=>a+l.anki.length,0);
  let html='<div class="hero"><div><h1 style="margin:4px 0 2px">Chalk Talks</h1>'+
    '<div class="stats">'+DATA.length+' lessons · '+nM+' board MCQs · '+nC+' Anki cards</div>'+
    '<div class="sub" style="margin-top:2px">Tap any topic for the full lesson; ▶ starts the presenter guide on your phone</div></div>'+
    '<div class="prog"><div class="lbl"><span><span style="color:var(--good);font-weight:700">'+nT+' taught</span>'+(nP?' · '+nP+' planned':'')+'</span><span>'+DATA.length+' total</span></div>'+
    '<div class="bar"><i style="width:'+(100*nT/DATA.length)+'%;background:var(--good)"></i><i style="width:'+(100*nP/DATA.length)+'%;background:var(--warm)"></i></div></div></div>'+
...
function drawChips(){
  ...
  let h='<button class="chip'+(homeState.tag==="All"&&!homeState.untaught?" on":"")+'" onclick="homeState.tag=\'All\';homeState.untaught=false;drawChips();drawList()">All<span class="ct">'+DATA.length+'</span></button>';
  TAGS.forEach((t,i)=>{const c=TAGCOL[t]||TAGCOL.General;const n=DATA.filter(l=>l.tag===t).length;
    h+='<button class="chip tagc'+(homeState.tag===t?" on":"")+'" style="--pb:'+c[0]+';--pt:'+c[1]+'" onclick="pickTag('+i+')">'+esc(t)+'<span class="ct">'+n+'</span></button>';});
...
function cardHTML(l){
  ...
  const c=TAGCOL[l.tag]||TAGCOL.General;
  return '<div class="card" style="--tc:'+c[1]+'" onclick="go(\'#/lesson/'+l.slug+'\')">'+
    '<div class="row1">'+pill(l.tag)+'<button class="state '+st+'" onclick="event.stopPropagation();cycle(\''+l.slug+'\')" title="Cycle planned / taught">'+mark+'</button></div>'+
    '<h3>'+esc(l.title)+'</h3>'+
    ...
    '<div class="foot"><button class="btn acc" style="flex:1" onclick="event.stopPropagation();go(\'#/teach/'+l.slug+'\')">▶ Presenter guide</button>'+
    '<button class="btn" onclick="go(\'#/lesson/'+l.slug+'\')">Lesson ›</button></div>'+
...
function drawList(){
  ...
  const vis=DATA.filter(l=>(homeState.tag==="All"||l.tag===homeState.tag)&&
    !(homeState.untaught&&(prog[l.folder]||{}).status==="taught"));
  if(!vis.length){box.innerHTML='<div class="empty">Nothing matches.</div>';return;}
  let h="";
  if(homeState.tag==="All"&&!homeState.untaught){
    const up=DATA.filter(l=>(prog[l.folder]||{}).status==="planned");
    if(up.length){h+='<div class="shead">Up next · planned</div>';
      up.forEach(l=>{h+='<div class="upnext" onclick="go(\'#/lesson/'+l.slug+'\')">'+pill(l.tag)+
        '<span class="t">'+esc(l.title)+'</span>'+(l.duration?'<span class="sub">'+esc(l.duration)+'</span>':'')+
        '<span style="flex:1"></span><button class="btn acc" onclick="event.stopPropagation();go(\'#/teach/'+l.slug+'\')">▶ Presenter guide</button></div>';});}
  }
  if(homeState.tag==="All"){
    TAGS.forEach(t=>{const ls=vis.filter(l=>l.tag===t);if(!ls.length)return;
      h+='<div class="shead">'+esc(t)+'<span class="cnt">'+ls.length+'</span></div><div class="grid" style="margin-top:8px">'+
        ls.map(cardHTML).join("")+'</div>';});
  }else{
    h+='<div class="grid">'+vis.map(cardHTML).join("")+'</div>';
  }
  box.innerHTML=h;
}
function cycle(slug){
  ...
  if(!next){delete prog[l.folder];}
    ...
    prog[l.folder]=rec;}
  saveProg();
  if((location.hash||"#/")==="#/"){renderHome();}else{render();}
}
...
function renderLesson(slugAnchor){
  ...
  const studentBtns=
    (onlineViewOk(l)?'<a class="btn warm" target="_blank" rel="noopener" href="'+officeUrl(l)+'">▶ PowerPoint online — best quality</a>':"")+
    (spdf?'<button class="btn" onclick="go(\'#/present/'+l.slug+'\')">▶ Slides in-app (PDF)</button>':"")+
    fl(f.presenter,"Download pptx");
  let h='<div class="lhead">'+pill(l.tag)+'<h1>'+esc(l.title)+'</h1>'+
    '<div class="sub">'+esc(l.subtitle)+'</div>'+
    '<div class="agroup"><div class="aglabel">Your guide — open on your phone (presenter only)</div>'+
    '<div class="actions"><button class="btn acc" onclick="go(\'#/teach/'+l.slug+'\')">▶ Presenter guide</button></div></div>'+
    (studentBtns?'<div class="agroup"><div class="aglabel">Student screen — put slides on a separate display</div>'+
      '<div class="actions">'+studentBtns+'</div></div>':"")+
    '<div class="agroup"><div class="aglabel">Materials</div><div class="actions">'+
    fl(f.onepager_pdf,"One-pager")+fl(f.handout,"Handout")+fl(f.anki,"Anki.txt")+
    '<a class="btn" href="'+enc(l.folder+"/"+l.package)+'">Original</a></div></div>'+
    '<div class="setuphint">How to run it: open the <b>Presenter guide</b> on your phone — it’s your private script, cases, and timer. Students should never see it. Show the slides on a separate screen they can see; try <b>PowerPoint online</b> first for the truest rendering.</div></div>'+
...
function drawTeach(){
  ...
  let body="";
  if(sl.type==="title"){body='<h2>'+esc(l.title)+"</h2><div class='sub'>"+esc(l.subtitle)+"</div>"+
    "<div class='sub' style='margin-top:8px'>Presenter guide — for your eyes only. Put the slides on a separate screen for the students.</div>"+
  ...
  document.getElementById("teachRoot").innerHTML=
    ...
    '<div class="nav"><button class="iconbtn" onclick="exitTeach()" aria-label="Exit presenter guide">✕</button>'+
